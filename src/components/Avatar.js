    ...
    return (
        <div style={style.wrapper}>
            <div style={style.box}>
                {settings ?
                    <>
                        <img onMouseEnter={e => setMouseEnter(true)}
                             onMouseLeave={e => setMouseEnter(false)}
                             style={style.img} src={img && img} alt=""/>
                        <input name='status' maxLength='20'
                               onChange={(e) => changeHandler(e)}
                               style={style.badge} value={status}/>
                    </>
                    :
                    <>
                        <img style={style.img} src={img && img} alt=""/>
                        {status ?  <p style={style.badge}>{status}</p> : null}
                    </>
                }
